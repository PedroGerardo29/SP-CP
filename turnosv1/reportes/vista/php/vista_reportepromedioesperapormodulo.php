...
if (isset($_GET['fecini']) && isset($_GET['fecfin']) && isset($_GET['modulo']) && isset($_GET['tipo'])) {
    ...
    $cuereporte = json_decode($vista->GenerarReporteTiempoPromedioEsperaPorModulos());
    if ($_GET['tipo'] == 'pdf') {
        ...
        $datacreator = array(
            ...
            'Subject' => 'REPORTE DE TIEMPO PROMEDIO DE ESPERA  POR MÓDULOS',
        ...
        $pdf->ezText("<strong>REPORTE DE TIEMPO PROMEDIO DE ESPERA  POR MÓDULOS\n</strong>", 13, array('justification' => 'center'));
        ...
        $stream_options = array(
            'Content-Disposition' => 'REPORTE_DE_TIEMPO_PROMEDIO_ESPERA_POR_MODULOS_' . date("d/m/y") . '_' . rand(0, 10000) . '.pdf',
            'compress' => 1,
        );
        ob_end_clean();
        ...
        if (count($cuereporte) == 0) {
            $objPHPExcel->setActiveSheetIndex(0)
                    ->setCellValue('B2', 'REPORTE DE TIEMPO PROMEDIO DE ESPERA POR MÓDULOS')
        ...
        header('Content-Disposition: attachment;filename="Reporte_de_Tiempo_Promedio_Espera_por_Modulos_' . $_GET['fecini'] . '_' . $_GET['fecfin'] . '_' . rand(100, 1000) . '.xlsx"');
