...
if (isset($_GET['fecini']) && isset($_GET['fecfin']) && isset($_GET['personal']) && isset($_GET['tipo'])) {
    ...
    $cuereporte = json_decode($vista->GenerarReporteTurnosPorUsuarios());
    if ($_GET['tipo'] == 'pdf') {
        ...
        $datacreator = array(
            ...
            'Subject' => 'REPORTE DE TURNOS ATENDIDOS  POR USUARIO',
        ...
        $pdf->ezText("<strong>REPORTE DE TURNOS ATENDIDOS  POR USUARIO\n</strong>", 13, array('justification' => 'center'));
        ...
        $stream_options = array(
            'Content-Disposition' => 'REPORTE_DE_TURNOS_ATENDIDOS_POR_USUARIOS_' . date("d/m/y") . '_' . rand(0, 10000) . '.pdf',
            'compress' => 1,
        );
        ob_end_clean();
        ...
        $objPHPExcel->getProperties()
                ...
                ->setTitle("Reporte Turnos Atendidos Por Usuarios")
                ->setSubject("Reporte Turnos Atendidos Por Usuarios")
                ->setDescription("Reporte Turnos Atendidos Por Usuarios")
                ->setKeywords("Turnos Usuarios")
                ->setCategory("Reporte Turnos Atendidos Por Usuarios");
        if (count($cuereporte) == 0) {
            $objPHPExcel->setActiveSheetIndex(0)
                    ->setCellValue('B2', 'REPORTE DE TURNOS ATENDIDOS  POR USUARIO')
        ...
        $objPHPExcel->getActiveSheet()->setTitle('TURNOS POR USUARIOS');
        $objPHPExcel->setActiveSheetIndex(0);
        header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
        header('Content-Disposition: attachment;filename="Reporte_de_Turnos_Atendidos_por_Usuarios_' . $_GET['fecini'] . '_' . $_GET['fecfin'] . '_' . rand(100, 1000) . '.xlsx"');
