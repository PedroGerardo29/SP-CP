...
if (isset($_GET['fecini']) && isset($_GET['fecfin']) && isset($_GET['modulo']) && isset($_GET['tipo'])) {
    ...
    $cuereporte = json_decode($vista->GenerarReporteTiempoPromedioAtencionPorModulos());
    if ($_GET['tipo'] == 'pdf') {
        ...
        $datacreator = array(
            ...
            'Subject' => 'REPORTE DE TIEMPO PROMEDIO DE ATENCIÓN  POR MÓDULOS',
        ...
        $pdf->ezText("<strong>REPORTE DE TIEMPO PROMEDIO DE ATENCIÓN  POR MÓDULOS\n</strong>", 13, array('justification' => 'center'));
        ...
        $stream_options = array(
            'Content-Disposition' => 'REPORTE_DE_TIEMPO_PROMEDIO_ATENCION_POR_MODULOS_' . date("d/m/y") . '_' . rand(0, 10000) . '.pdf',
            'compress' => 1,
        );
        ob_end_clean();
        ...
        $objPHPExcel->getProperties()
                ...
                ->setTitle("Reporte Tiempo Espera Por Módulos")
                ->setSubject("Reporte Tiempo Espera Por Módulos")
                ->setDescription("Reporte Tiempo Espera Por Módulos")
                ->setKeywords("Tiempo Espera Módulos")
                ->setCategory("Reporte Tiempo Espera Por Módulos");
        if (count($cuereporte) == 0) {
            $objPHPExcel->setActiveSheetIndex(0)
                    ->setCellValue('B2', 'REPORTE DE TIEMPO PROMEDIO DE ATENCIÓN  POR MÓDULOS')
        ...
        $objPHPExcel->getActiveSheet()->setTitle('PROMEDIO ATENCIÓN POR MÓDULOS');
        $objPHPExcel->setActiveSheetIndex(0);
        header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
        header('Content-Disposition: attachment;filename="Reporte_de_Tiempo_Promedio_por_Modulos_' . $_GET['fecini'] . '_' . $_GET['fecfin'] . '_' . rand(100, 1000) . '.xlsx"');
